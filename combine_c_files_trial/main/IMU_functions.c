...
static struct IMU_data_storage IMU_data {
    uint16_t accel_x[50];
    uint16_t accel_y[50];
    uint16_t accel_z[50];
    uint16_t gyro_x[50];
...
void save_array(void)
{
    ...
    size_t size = 50 * sizeof(uint16_t);
    
    err = save_blob_nvs(IMU_data->accel_x, "accel_x", size);
    err = save_blob_nvs(IMU_data->accel_y, "accel_x", size);
    err = save_blob_nvs(IMU_data->accel_z, "accel_x", size);
    err = save_blob_nvs(IMU_data->gyro_x, "gyro_x", size);
    err = save_blob_nvs(IMU_data->gyro_y, "gyro_y", size);
    err = save_blob_nvs(IMU_data->gyro_z, "gyro_z", size);

    err = save_value_nvs16(average(IMU_data->gyro_x), "avg_rps");
    err = save_value_nvs16(max(IMU_data->gyro_x), "peak_rps");
    err = save_value_nvs32(0x01, "lat_init");
    err = save_value_nvs32(0x02, "lat_final");
    err = save_value_nvs32(0x01, "long_init");
    err = save_value_nvs32(0x02, "long_final");
    err = save_value_nvs32(0x01, "alt_init");
    err = save_value_nvs32(0x02, "alt_final");
}
